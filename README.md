...
- **双向定位高亮（源码版）**：点击 JSON 的 key/value 或协议、Host、路径、Hash，左侧高亮对应 URL 原文；点击 URL 则高亮对应字段，并滚动到可见位置。重复参数的数组元素逐项对应，高亮不改变输入焦点或文本。
- **Hash 解析与编辑（源码版）**：单独显示 `#` 开始的完整内容，支持锚点和 `#/route?tab=info` 形式的路由。修改时可省略开头的 `#`，清空即移除 Hash，单独输入 `#` 保留空 Hash。Hash 内的 `?`、`&` 不会混入 Query JSON。
...
当前源码开发版还支持直接编辑协议、Host、路径和 Hash（空白状态也可创建 URL），并更新了扁平工具栏和局部文本／配色刷新。通用应用约 0.74 MiB；编辑耗时及测量限制见[本次优化记录](docs/optimization-2026-09-20.md)。这些更新尚未包含在 v1.0.0 下载包中，可从源码构建。
